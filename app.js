...
if (localStorage.getItem('isVIP') === null) localStorage.setItem('isVIP', 'false');
...
headerButtons += `<button onclick="toggleVIP()" style="background-color: ${isVIP ? '#002D72' : '#E5E7EB'}; color: ${isVIP ? 'white' : '#111827'}; border: 1px solid #D1D5DB; padding: 8px 12px; border-radius: 8px; font-weight: bold; cursor: pointer; margin-left: 10px;">${isVIP ? '👑 VIP' : 'Test VIP'}</button>`;
document.querySelector('header').innerHTML += `<div style="display:flex;">${headerButtons}</div>`;

function toggleVIP() { isVIP = !isVIP; localStorage.setItem('isVIP', isVIP.toString()); location.reload(); }
...
// АДМИН ПАНЕЛЬ (С НОВЫМ ПОЛЕМ ДЛЯ ПИТЧЕРОВ)
...
async function publishBoard() {
    if(confirm("Are you sure you want to GO LIVE with today's board? This will replace yesterday's games for all users.")) {
...
async function loadMatches() {
    ...
    // Бэкенду нужно знать, босс ли ты, чтобы отдать правильные матчи
    ...
    try {
        ...
        matches.forEach(match => {
            ...
            // Если босс и матч не опубликован, показываем пометку "DRAFT"
            ...
            // Логика питчеров: Сначала берем твои ручные, если их нет — официальные. 
            // Если и тех нет (или бэкенд их стер для вчерашних игр) — ничего не выводим.
...
// ... остальной код модалок и чата остается без изменений ...
...
function renderChatControls() {
    ...
    if (!isVIP) {
        controls.innerHTML = `
            ...
            <button onclick="toggleVIP()" style="background:#D50032; color:white; border:none; padding:12px 20px; border-radius:8px; font-weight:bold; cursor:pointer;">Unlock VIP</button>`;
...
async function sendChatMessage() {
    if (!isVIP || messagesLeft <= 0) return;
